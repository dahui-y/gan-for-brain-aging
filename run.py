...
class experiment(object):
    def __init__(self, comet_exp=None):
        ...
        self.repo = pygit.Repo(search_parent_directories=True)
    ...
    def get_conf(self, args):
        ...
        try:
            ...
            self.log.error("Error loading configuration: {}".format(e))
            raise
    ...
    def get_executor(self, conf):
        ...
        # return executor
        try:
            self.log.info("Initializing model: {}".format(conf.model))
            module_name = conf.model.split('.')[0]
            model_name = conf.model.split('.')[1]
            model = getattr(importlib.import_module("models." + module_name), model_name)(conf)
            model.build()
    
            self.log.info("Initializing executor: {}".format(conf.executor))
            module_name = conf.executor.split('.')[0]
            model_name = conf.executor.split('.')[1]
            executor = getattr(importlib.import_module("model_executor." + module_name), model_name)(conf, model, self.comet_exp)
            return executor
        except Exception as e:
            self.log.error("Error initializing model or executor: {}".format(e))
            raise
...
#     comet_exp = Experiment(api_key="nePjHU0c5H50l9foRSFLJqpRS",project_name="gan-for-brain-aging",workspace="dahui-y" )
#     exper = experiment(comet_exp)
#     exper.run()
if __name__ == "__main__":
    # Add the following code anywhere in your machine learning file
    api_key = "nePjHU0c5H50l9foRSFLJqpRS"
    project_name = "gan-for-brain-aging"
    workspace = "dahui-y"
